...
class RiotApiEndpoints {
  ...
  getPUUID = (gameName: string, tagLine: string, region: string = "americas") =>
    `https://${region}.api.riotgames.com/riot/account/v1/accounts/by-riot-id/${gameName}/${tagLine}`;
  getMatchHistory = (puuid: string, count: number = 100) =>
    `${this.BASE_URL}/lol/match/v5/matches/by-puuid/${puuid}/ids?start=0&count=${count}`;
  getMatchDetails = (matchId: string) =>
    `${this.BASE_URL}/lol/match/v5/matches/${matchId}`;
  getAllMatches = (puuid: string, countNumber?: number) =>
    `${this.BASE_URL}/lol/match/v5/matches/by-puuid/${puuid}/ids?start=0&count=${countNumber}`;
...
export class RiotAPIService implements HasApiHelth {
  ...
  async getRegions() {
      ...
      const response = await axios.get(url, {headers: this.headers });
      if (!response) throw new Error("Failed to fetch regions");
    console.log(
