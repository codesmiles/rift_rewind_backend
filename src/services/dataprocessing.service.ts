...
export class DataProcessorService {
  ...
  private extractPlayerMatches(
    matches: any[],
    puuid: string,
  ): ProcessedMatch[] {
    if (matches.length < 1) throw new Error("No matches found");
  ...
  private analyzeTiltPatterns(matches: ProcessedMatch[]): TiltAnalysis {
    ...
    return {
      tiltRecoveryRate:
        gamesAfterLoss > 0 ? (winsAfterLoss / gamesAfterLoss) * 100 : 0,
      gamesAfterLoss,
      winsAfterLoss,
      avgPerformanceAfterLoss:
        gamesAfterLoss > 0 ? totalKDAAfterLoss / gamesAfterLoss : 0,
    };
  }
