...
router.post("/wrapped/generate", async (req: Request, res: Response) => {
  try {
    ...
    // // Generate insights
    const wrapped = await insightsService.generateFullWrapped(
      matches,
      puuid,
      gameName,
      tagLine,
...
router.post("/quick-stats", async (req: Request, res: Response) => {
  try {
    ...
    const processor = new (
        await import("../services/dataprocessing.service")
...
router.get("/regions", async (req: Request, res: Response) => {
  try {
    const region = await riotAPI.getRegions();
    const response = new ResponseFormat(true, region);
    return res.json(response.toJson());
  } catch (error: any) {
    const response = new ResponseFormat(false, error.message);
    return res.status(500).json(response.toJson());
...
router.get("/ai/story", async (req: Request, res: Response) => {
  ...
  try {
    ...
    const story = await insightsService.generateFullWrapped(
      matches,
      puuid,
      gameName,
      tagLine,
...
router.post("/summoner/validate", async (req: Request, res: Response) => {
  try {
    ...
    const response = new ResponseFormat(false, JSON.stringify(error.message));
    return res.status(500).json(response.toJson());
  }
});
...
router.get("/health", async (req: Request, res: Response) => {
  const response = new ResponseFormat(true, {
    "Riot API Health": await riotAPI.health(),
    // "google Health": await new GoogleAIService().health(),
