...
import {
  ...
  ResponseMessageEnum,
  PaginatedResponse,
} from "../utils"; // Assuming these are defined elsewhere
...
abstract class BaseAbstract<T, I extends Document> {
  ...
  abstract findOrCreate(
    ...
    key: keyof I,
    options?: { populate?: string[] },
  ): Promise<I>; // Added options
  abstract findMany(
    filter: { key: keyof I; values: any[] },
    options?: { populate?: string[] },
  ): Promise<I[]>; // Refined filter type
  abstract findManyOrCreateMany(
    identifiers: any[],
    key: keyof I,
    options?: { populate?: string[] },
  ): Promise<I[]>; // Added options, refined identifiers type
...
type BaseServiceConstructorType<I extends Document> = {
  // I extends Document here too
  Model: Model<I>;
  serializer?: string[];
...
export class BaseService<T, I extends Document> extends BaseAbstract<T, I> {
  // I extends Document here
  ...
  public constructor(builder: BaseServiceConstructorType<I>) {
    ...
    this.allowedOperations =
      builder.allowedOperations || Object.values(CrudOperationsEnum);
  }
  ...
  async create(payload: T, options?: { populate?: string[] }): Promise<I> {
    ...
    if (options?.populate?.length) {
      ...
      create = await create.populate(
        this._getPopulateOptions(options.populate),
      );
    }
    return create;
  }
  ...
  async getAll(
    ...
    const formattedQueries = {} as Record<
      string,
      MongoFilters<I | T> | string | any
    >; // Use 'any' as a fallback if types are complex
    for (const [key, value] of Object.entries(mongoFilters)) {
      ...
      if (schemaPath) {
        // Only process if it's a schema path
        const isArrayField =
          schemaPath instanceof Schema.Types.Array ||
          schemaPath.instance === "Array";
        formattedQueries[key] = isArrayField
          ? { $in: Array.isArray(value) ? value : [value] }
          : value;
      } else {
        // Handle query operators that are not schema paths
    ...
    if (
      Object.values(formattedQueries).some((val) => val === ConstantEnums.NULL)
    ) {
      return {
        payload: [],
        meta: {
  ...
  async findMany(
    ...
    const mongoFilter = this._getBaseQuery({
      [filter.key]: { $in: filter.values },
    } as FilterQuery<I>);

    let queryChain = this.Model.find(mongoFilter).select(
      this._excludedFieldsProjection,
    );
    ...
    const missing_keys = filter.values.filter(
      (value) => !foundValues.has(String(value)),
    );

    if (missing_keys.length > 0) {
      throw new Error(
        `${ResponseMessageEnum.MISSING_DATABASE_KEYS}: ${missing_keys.join(", ")}`,
      );
    }
  ...
  async findManyOrCreateMany(
    ...
    if (
      !this.allowedOperations.includes(
        CrudOperationsEnum.FIND_MANY_OR_CREATE_MANY,
      )
    ) {
    ...
    const filter = this._getBaseQuery({
      [key]: { $in: identifiers },
    } as FilterQuery<I>);
    let existingDocs = await this.Model.find(filter)
      .select(this._excludedFieldsProjection)
      .exec();
    const existingValues = new Set(
      existingDocs.map((doc) => doc[key] as string),
    );
    ...
    if (toCreate.length > 0) {
      createdDocs = (await this.Model.insertMany(
        toCreate.map((val) => ({ [key]: val })),
        { ordered: false },
      )) as unknown as I[]; // insertMany returns an array of documents
    }

    if (options?.populate?.length) {
      existingDocs = await this.Model.populate(
        existingDocs,
        this._getPopulateOptions(options.populate),
      );
      if (createdDocs.length > 0) {
        createdDocs = await this.Model.populate(
          createdDocs,
          this._getPopulateOptions(options.populate),
        );
      }
  ...
  async search(query: string): Promise<I[] | null> {
    ...
    const projection: ProjectionType = Object.fromEntries(
      this.serializer.map((field) => [field, 0]),
    );
    projection.score = { $meta: "textScore" };
  ...
  async syncIndexes(): Promise<void> {
    if (!this.allowedOperations.includes(CrudOperationsEnum.SYNC_INDEXES)) {
      // Added check
  ...
  async dropIndexes(): Promise<void> {
    if (!this.allowedOperations.includes(CrudOperationsEnum.DROP_INDEXES)) {
      // Added check
